...
# 強化学習法を実行する方法は大きく分けて 3 個あります．動的計画法，モンテカルロ法，時間的差分法（Temporal Difference（TD）法）です．動的計画法は正確な環境モデルを持っている場合に最適な方策を決定することが可能な方法です．エピソードを終了せずとも方策を最適化しようとすることが可能です．モンテカルロ法は完全な環境モデルを持たない場合，行動の結果として報酬が得られるかどうかが不明な際に用いられる方法です．エージェントの行動に応じて学習を進めます．エピソードを終了させることではじめて報酬を計算します．これに対して，時間的差分法は上述のふたつを組み合わせたような方法です．行動に対して得られる実際の報酬から評価関数を改善することで最適な方策を見つけようとする方法です．この 3 個の方法の中では時間的差分法が最も現実的な学習法ではないでしょうか．
# 
# この章で紹介する方法は時間的差分法です．時間的差分法にはいくつかの種類があります．
# 
