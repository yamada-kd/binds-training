...
# 教師あり学習法では，入力データとそれに紐づいた教師データの組み合わせ利用する．例えば，画像認識の問題を考えるとすると，「ゴリラが写っている画像」と共に「ゴリラ」という情報を用いる．この場合，「ゴリラが写っている画像」が成長させたい人工知能に入力するデータで，「ゴリラ」という情報は人工知能にこの画像にはゴリラが写っていることを教えるための教師（supervisor）データである．この教師データはラベル，正解，ターゲットデータ等とも呼ばることがある．このような入力と教師データのペアをいくつも用意し，それらを人工知能アルゴリズムに学習させる．ここで「入力データ」という用語は明確に意味のある専門用語として利用している．人工知能アルゴリズムに入力するデータのことをスカラ，ベクトル，行列等の区別をせずに，入力データと呼ぶ（文脈によっては「入力値」とも表現する）．よって，「入力データを人工知能アルゴリズムに入力する」というような表現が以降で出現することがある．教師あり学習の目的は，そのようにして構築した人工知能に新たな入力データが入力された場合に，そのラベルを予測させることである．ここでは，ラベルを予測させる分類問題を例にしたが，数値を予測させることも可能であり，回帰問題にも利用可能である．教師あり学習法で成長させられる人工知能アルゴリズムとしては，ニューラルネットワーク，サポートベクトルマシン，線形回帰式，決定木をはじめとして様々なものがある．

# ### 教師なし学習法

# 一方で，教師なし学習法で成長させた人工知能は入力データに対して何かの予測値を出力するようなものではない（最終的な産物としてそのような出力をするように設計することも場合によっては可能）．教師なし学習法では教師あり学習法で用いるような入力データと教師データの組み合わせを利用しない．教師なし学習法で人工知能アルゴリズムに学習させるデータは入力データのみである．教師なし学習法で成長させられる人工知能アルゴリズムは与えられたデータに存在する傾向を把握するために用いられることが多い．類似の性質を持つデータを集めるクラスタリングや，人の能力では理解できないような高次元に存在するデータを可視化することや，データの特徴の抽出等に利用される．教師なし学習法のアルゴリズムとして用いられるものとしては，$\it{k}$-means，階層的クラスタリング法（単連結法をはじめとする階層的にデータを分類する手法の総称），主成分分析法，自己組織化マップ（ニューラルネットワークの一種）等の様々なものがある．

# ### 強化学習法

# 強化学習法は教師あり学習法と完全に独立な学習法ではない．正解の情報を利用するという点においては教師あり学習法と同様であり，教師あり学習法の一種と考えることも可能である．しかし実際の強化学習法で行っていることは教師あり学習法で行っていることと様々な部分で異なっているように見え，現状の分類では強化学習法と教師あり学習法は別々に紹介される場合が多いため，本書でもそれに倣う．強化学習法で用いる正解の情報は報酬と呼ばれる．これに加えて強化学習法では，エージェントと呼ばれる何らかの行動をする存在を設定する．エージェントは何らかの環境に存在しており，一連の行動をすることによって得られる報酬を最大化するように成長させられる．このときの報酬は教師あり学習法で用いられるような正解データのような即時的に正解または不正解を判断できるようなものではなく，一連の行動の結果に対して与えられるものである．この点において，従来の教師あり学習法と強化学習法は異なる．例えば，2人のキャラクター同士が何らかのステージで戦う格闘ゲームを例にすると，最大の報酬が与えられるのは敵であるキャラクターを倒したときであって，戦いの過程においてパンチを繰り出せとか，必殺技を放て等の個々の行動に対する正解は提示されない．強化学習法は現在最も開発研究が行われている学習法であると言っても過言ではなく，日々様々な手法が考案されている．例えば，ディープニューラルネットワークを利用する深層強化学習アルゴリズムのdeep Q-networkは一部のゲームにおいて人以上の性能を示している．強化学習法で利用されるアルゴリズムとしては，マルコフ決定過程，動的計画法，モンテカルロ法，Q学習法等がある．
