...
# 機械学習の分野では日々新しい言葉が作られていたり，様々な分野から機械学習の研究をしようとしている人がいたりするため，用語の用法にコンセンサスが取れていない場合があります．そこでこの説では機械学習に関する用語の説明をします．
...
# 人工知能にデータを読ませ成長させる過程を学習と言いますが，学習方法にはいくつかの種類があります．機械学習法は教師あり学習法（supervised learning），教師なし学習法（unsupervised learning），強化学習法（reinforcement learning）の3つに大別されます．これらの学習法は，どれがどれに対して優れていて，どれがどれに対して劣っている等の比較をするような対象ではありません．これらは，与えられた問題を解決するためにどのような人工知能を構築したいのかという観点からそれに適した方法を選択するものです．

# ### 教師あり学習法

# 教師あり学習法では，入力データとそれに紐づいた教師データの組み合わせ利用します．例えば，画像認識の問題を考えるとすると，「ゴリラが写っている画像」と共に「ゴリラ」という情報を用います．この場合，「ゴリラが写っている画像」が成長させたい人工知能に入力するデータで，「ゴリラ」という情報は人工知能にこの画像にはゴリラが写っていることを教えるための教師（supervisor）データです．この教師データはラベル，正解，ターゲットデータ等とも呼ばることがあります．このような入力と教師データのペアをいくつも用意し，それらを人工知能アルゴリズムに学習させます．教師あり学習の目的は，そのようにして構築した人工知能に新たな入力データが入力された場合に，そのラベルを予測させることです．ここでは，ラベルを予測させる分類問題を例にしましたが，数値を予測させることも可能で，回帰問題にも利用可能です．教師あり学習法で成長させられる人工知能アルゴリズムとしては，ニューラルネットワーク，サポートベクトルマシン，線形回帰式，決定木をはじめとして様々なものがあります．

# ### 教師なし学習法

# 教師なし学習法で成長させた人工知能は入力データに対して何かの予測値を出力するようなものではありません（最終的な産物としてそのような出力をするように設計することも場合によっては可能）．教師なし学習法では教師あり学習法で用いるような入力データと教師データの組み合わせを利用しません．教師なし学習法で人工知能アルゴリズムに学習させるデータは入力データのみです．教師なし学習法で成長させられる人工知能アルゴリズムは与えられたデータに存在する傾向を把握するために用いられることが多いです．類似の性質を持つデータを集めるクラスタリングや，人の能力では理解できないような高次元に存在するデータを可視化することや，データの特徴の抽出等に利用されます．教師なし学習法のアルゴリズムとして用いられるものとしては，$\it{k}$-means，階層的クラスタリング法（単連結法をはじめとする階層的にデータを分類する手法の総称），主成分分析法，自己組織化マップ（ニューラルネットワークの一種）等の様々なものがあります．

# ### 強化学習法

# 強化学習法は教師あり学習法と完全に独立な学習法ではないと思います．正解の情報を利用するという点においては教師あり学習法と同様で，教師あり学習法の一種と考えることも可能なのではないでしょうか．しかし実際の強化学習法で行っていることは教師あり学習法で行っていることと様々な部分で異なっているように見え，現状の分類では強化学習法と教師あり学習法は別々に紹介される場合が多いです．強化学習法で用いる正解の情報は報酬と呼ばれます．これに加えて強化学習法では，エージェントと呼ばれる何らかの行動をする存在を設定します．エージェントは何らかの環境に存在していて，一連の行動をすることによって得られる報酬を最大化するように成長させられます．このときの報酬は教師あり学習法で用いられるような正解データのような即時的に正解または不正解を判断できるようなものではなく，一連の行動の結果に対して与えられるものです．この点において，教師あり学習法と強化学習法は異なります．例えば，2人のキャラクター同士が何らかのステージで戦う格闘ゲームを例にすると，最大の報酬が与えられるのは敵であるキャラクターを倒したときであって，戦いの過程においてパンチを繰り出せとか，必殺技を放て等の個々の行動に対する正解は提示されません．強化学習法は現在最も活発に開発研究が行われている学習法であると言っても過言ではなく，日々様々な手法が考案されています．例えば，ディープニューラルネットワークを利用する深層強化学習アルゴリズムの deep Q-network は一部のゲームにおいては人以上の性能を示しています．強化学習法で利用されるアルゴリズムとしては，マルコフ決定過程，動的計画法，モンテカルロ法，Q学習法等があります．
...
# 機械学習をしている最中において，人工知能は与えられたデータを繰り返し学習することでそのデータに自身を適合させます．そして，構築された人工知能の役割はその「与えられたデータ」以外の新たな別のデータに対して良い予測性能を発揮できることです．「与えられたデータ」と「新たな別のデータ」の性質が同等とは限りません．もし人工知能が「与えられたデータ」に過剰適合してしまい，「新たな別のデータ」に対して良い予測性能を発揮できないとき，その人工知能は過学習を起こしているというように表現されます．人工知能の開発の際に最も大きな問題となるのが過学習です．人工知能開発に要する時間の大半は過学習を起こさないための工夫をしている時間であると言っても過言ではないです．
...
# ```{note}
# バリデーションデータセットは開発（デベロップメント）データセット（development dataset）とも言います．
# ```
